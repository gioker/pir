...
double IrisCode::compare_with(IrisCode* other){
	...
	Bitcode32 temp[64];
	int top=0; //differences of two images, without irrelevant regions (e.g. eyelid)
	int bottom=0; //common relevant parts of both masks
...
We note that we may wish to form a better formula, talk to Duncan about this */
   ...
   if(bottom == 0){
	 ...
	 if(top == 0){
        ...
        cout << "2048 - bottom: " << static_cast<double>(2048) - bottom << endl;
        cout << "(2048 - bottom)/2048: " << (static_cast<double>(2048) - bottom)/2048 << endl;
