...
void MainView::on_searchButton_clicked() {
	...
	QString matchingIris;
	QString resultText;
	if(Database::search(curFile, view->getIris()->getBitCode(), view->getIris()->getBitCodeMask(), hamDist, matchingIris)) {
...
bool MainView::loadImage(QString fullFileName, bool add) {
	bool result = false;
	curFile = QFileInfo(fullFileName).fileName();
	result = view->loadImage(fullFileName);

//  std::cout << QFileInfo(curFile).baseName().toUtf8().constData() << std::endl;
	// Uncomment this to make it automatically add to the database
	double hamDist;
	QString matchingIris;
	if(result) {
		if (add) {
			Database::add(curFile, view->getIris()->getBitCode(), view->getIris()->getBitCodeMask(), hamDist, matchingIris);
		}
		else {
			Database::search(curFile, view->getIris()->getBitCode(), view->getIris()->getBitCodeMask(), hamDist, matchingIris);
		}
	}
	return result;
}
